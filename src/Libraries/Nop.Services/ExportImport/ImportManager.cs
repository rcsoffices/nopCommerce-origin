...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ImportManager : IImportManager
    {
        ...
        public virtual async Task ImportCustomersFromXlsxAsync(Stream stream)
        {
            ...
            var manager = new PropertyManager<Customer, Language>(defaultProperties, _catalogSettings, localizedProperties, languages);

            var iRow = 2;
            var rolesToSave = new List<int>();
            var allRoles = await _customerService.GetAllCustomerRolesAsync();
            var counties = await _countryService.GetAllCountriesAsync();
                ...
                foreach (var property in manager.GetDefaultProperties)
                {
                    switch (property.PropertyName)
                    {
                        ...
                        case "CustomerRoles":
                            ...
                            break;
                        case "CreatedOnUtc":
                            customer.CreatedOnUtc = property.DateTimeNullable ?? DateTime.UtcNow;
                            break;
                        case "FirstName":
                        ...
                        case "Country":
                            ...
                            {
                                var country = counties.FirstOrDefault(c =>
